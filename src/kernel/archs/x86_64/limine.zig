...
pub export var kaddr: impl.ExecutableAddressRequest = .{};
pub export var rsdp: impl.RsdpRequest = .{};
pub export var kernel: impl.ExecutableFileRequest = .{};
...
pub fn dumpMmap() void {
    if (mmap.response) |m| {
        ...
        for (0..m.entry_count) |i| {
            const entry = m.getEntries()[i];
            log.debug("|{s: ^24} | 0x{x:0>16} | 0x{x:0>16} |", .{ std.enums.tagName(impl.MemoryMapType, entry.type).?, entry.base, entry.base + entry.length });
        }
        log.debug("+-------------------------------------------------------------------+", .{});
    }
}
