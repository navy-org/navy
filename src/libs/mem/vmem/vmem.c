...
static char *strncpy(char *s1, char const *s2, size_t n) {
  size_t i = 0;
 ...
 * we can calculate the freelist index by substracting the leading zero count
 * from 64 For example, the size 4096. clzl(4096) is 51, 64 - 51 is 13. We then
 * need to substract 1 from 13 because 2^13 equals 8192.
 */
...
/* We need to keep a global freelist of segments because allocating virtual
 memory (e.g allocating a segment) requires segments to describe it. (kernel
 only) In non-kernel code, this is handled by the host `malloc` and `free`
...
static VmemSegment *seg_alloc(void) {
  /* TODO: when bootstrapped, allocate boundary tags dynamically as described in
   * the paper */
  VmemSegment *vsp;
...
static int repopulate_segments(Allocator *backing_allocator) {
  struct {
    VmemSegment segs[64];
  } *segblock;
  size_t i;
...
static int seg_fit(VmemSegment *segment, size_t size, size_t align,
   ...
   *  For example, if `start` is 260, `phase` is 8 and align is `64`, we need to
   * do the following calculation: ALIGN_UP(260 - 8, 64) = 256. 256 + 8 = 264.
   * (264 % 64) is 8 as requested.
   */
  start = VMEM_ALIGNUP(start - phase, align) + phase;

  /* If for some reason, start is smaller than the segment base, we need to
   * ensure start is atleast as big as `align` This can happen if, for example,
   * we find that `start` is 0 and segment->base is 0x1000. In this case, align
   * is 0x1000. */
  ...
  /* Ensure that `end` is bigger than `start` and we found a segment of the
   * proper size */
...
int vmem_init(Vmem *ret, const char *name, void *base, size_t size,
              size_t quantum, VmemAlloc *afunc, VmemFree *ffunc, Vmem *source,
...
void vmem_destroy(Vmem *vmp) {
  ...
  TAILQ_FOREACH(seg, &vmp->segqueue, segqueue) { seg_free(seg); }
}
...
void *vmem_xalloc(Vmem *vmp, size_t size, size_t align, size_t phase,
                  size_t nocross, void *minaddr, void *maxaddr, int vmflag, Allocator *backing_allocator) {
  ...
  if (!(vmflag & VM_BOOTSTRAP))
    ASSERT(repopulate_segments(backing_allocator) == 0);
  ...
  while (true) {
    ...
    {
      /* If the size is not a power of two, use freelist[n+1] instead of
       * freelist[n] */
      if ((size & (size - 1)) != 0) {
        first_list++;
      }

      /* We just get the first segment from the list. This ensures constant-time
       * allocation. Note that we do not need to check the size of the segments
       * because they are guaranteed to be big enough (see freelist_for_size)
       */
      for (list = first_list; list < end; list++) {
        seg = LIST_FIRST(list);
        if (seg != NULL) {
          if (seg_fit(seg, size, align, phase, nocross, (uintptr_t)minaddr,
                      (uintptr_t)maxaddr, &start) == 0)
            goto found;
        }
      }
    }
    ...
    {
      /* TODO: Should we bother going through the entire list to find the
       * absolute best fit? */

      /* We go through every segment in every list until we find the smallest
       * free segment that can satisfy the allocation */
      for (list = first_list; list < end; list++)
...
found:
  ...
  if (seg->base != start) {
    /* If the start is not the base of the segment, we need to create another
     * segment; new_seg2 is a free segment that starts at `base` and ends at
     * `start-base`. We also need to make make `seg` start at `start` and reduce
     * its size. For example, if we allocate a segment [0x100, 0x1000] in a [0,
     * 0x10000] span, we need to split [0, 0x10000] into [0x0, 0x100] (free),
     * [0x100, 0x1000] (allocated), [0x1000, 0x10000] (free). In this case,
     * `base` is 0 and `start` is 0x100. This would create a segment with size
     * 0x100-0 that starts at 0.
     */
    new_seg2->type = SEGMENT_FREE;
    new_seg2->base = seg->base;
    new_seg2->size = start - seg->base;

    /* Make `seg` start at `start`, following the example, this would make
     * `(seg->base)` 0x100 */
    seg->base = start;

    /* Since we offset the segment by `start-(seg->base)`, we need to reduce
     * `seg`'s size */
  ...
  if (seg->size != size && (seg->size - size) > vmp->quantum - 1) {
    ...
    /* In the case where the segment's size is bigger than the requested size,
     * we need to split the segment into two: one free part of size `seg->size -
     * size` and another allocated one of size `size`. For example, if we want
     * to allocate [0, 0x1000] and the segment is [0, 0x10000], we have to
     * create a new segment, [0, 0x1000] and offset the current segment by
     * `size`. Therefore ending up with: [0, 0x1000] (allocated) [0x1000,
     * 0x10000] */
...
void vmem_xfree(Vmem *vmp, void *addr, size_t size) {
  VmemSegment *seg = NULL, *neighbor = NULL;
  VmemSegList *list;
